...
st.markdown(f"There are **{n_rows}** observations and **{n_cols - 1}** features in this dataset. The target variable is **{config['target']}.**")
...
drop_cols = st.sidebar.multiselect("Please select features to exclude", options=original_features)
...
thresh = st.sidebar.slider("Please select a classification threshold", min_value=0., max_value=1., value=0.5, step=0.05)

train_pred = m.predict_proba(X_train)[:, 1]
valid_pred = m.predict_proba(X_valid)[:, 1]
scores = pd.DataFrame(
    {
        "Accuracy": [metrics.accuracy_score(y_train, train_pred > thresh), metrics.accuracy_score(y_valid, valid_pred > thresh)],
        "ROC AUC": [
...
conf_matrix = pd.DataFrame(metrics.confusion_matrix(y_valid, valid_pred > thresh), columns=columns, index=index)
...
if st.checkbox("Calculate feature importances"):
    with st.spinner("Calculating the feature importance..."):
        permutation_feat_imp_valid = permutation_importance(m, X_valid.values, y_valid, n_repeats=10, n_jobs=-1)
